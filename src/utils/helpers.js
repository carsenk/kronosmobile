...
const denariuspos = require("denariusjs-pos-lib");
const denariusMessage = require("denariusjs-message");
const bip21d = require("bip21d");
const bip32d = require("bip32d");
...
const createTransaction = ({ address = "", transactionFee = 2, amount = 0, confirmedBalance = 0, utxos = [], blacklistedUtxos = [], changeAddress = "", wallet = "wallet0", selectedCrypto = "bitcoin", message = "", addressType = "bech32", setRbf = true } = {}) => {
	return new Promise(async (resolve) => {
		try {
			...
			if (selectedCrypto === "denarius") {
				const rootd = bip32d.fromSeed(seed, network);
				let txb = new denariuspos.TransactionBuilder(denariuspos.networks.denarius);
				...
				for (let i = 0; i < utxosLength; i++) {
					try {
						...
						const keyPair = rootd.derivePath(path);
				...
				for (let i = 0; i < utxosLength; i++) {
					try {
						...
						const key2 = denariuspos.ECPair.fromWIF(privkey);
						
						if (addressType === "legacy") {txb.sign(denariuspos.networks.denarius, index, key2);}
...
const openTxId = (txid = "", selectedCrypto = ""): void => {
	...
	if (selectedCrypto === "denarius") url = `https://chainz.cryptoid.info/d/tx.dws?${txid}`;
	if (selectedCrypto === "denariusTestnet") url = `https://chainz.cryptoid.info/d-test/tx.dws?${txid}`;
	openUrl(url);
};
...
const getBaseDerivationPath = ({ keyDerivationPath = "44", selectedCrypto = "denarius" }) => {
	try {
