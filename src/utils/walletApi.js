	...
	try {
		...
		switch (selectedCrypto) {
			...
			case "litecoinTestnet":
				...
				break;
			case "denarius":
				response = await fetch(`https://denarius.pro/api/tx/send`, fetchData("POST", { tx_hex: rawTx }));
				response = await response.json();
				response = response.status === "success" ? response.data.txid : "";
				break;
			case "denariusTestnet":
				response = await fetch(`https://test.denarius.pro/api/tx/send`, fetchData("POST", { tx_hex: rawTx }));
				response = await response.json();
				response = response.status === "success" ? response.data.txid : "";
				break;
		}
